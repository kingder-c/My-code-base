...
dojo.addOnLoad(function () {
    ...
    dojo.declare("modules.ClusterLayer", null, {
        ...
        constructor: function (data, options) {
            ...
            this.rgb = this.options.rgb || [165, 76, 91];
            ...
            var id = this.options.id || ("ClusterGraphicsLayer")
            ...
            for (var key in dataArray) {
                if (dataArray.hasOwnProperty(key)) {
					...
					*/
                    ...
                    //add by liuxu @2017-11-21 if处理    //edict lishiwu  @2017.12.25 ,点击弹窗隐藏，图片选择
                    if (this.options.sym && pointCount == 1) {                     
                        //var infoTemplate = new esri.InfoTemplate("消火栓属性", "管点编号: ${bmid} <br/>口径(mm): ${bmkj} <br/>状态: ${type}<br/>填埋方式: ${bmtm}<br/>最后养护时间: ${bmyhsj}<br/>地址: ${bmdz}<br/>");
                        dataArray[key].attr = dataArray[key].attr || { bmid: '', type: '', bmkj: '', bmtm: '', bmyhsj: '', bmdz: '' };
                        //根据属性判断，选取什么图片
                        var urlPic = "../images/hydrant/bz-normal.png";                    
                        if (dataArray[key].attr.type=="正常") {                           
                        } else if (dataArray[key].attr.type == "在建") {
                            urlPic = "../images/hydrant/bz-construction.png";                          
                        } else {
                            urlPic = "../images/hydrant/bz-discard.png";                          
                        }
                      
                        sym = new esri.symbol.PictureMarkerSymbol({
                            "url": urlPic,
                            "height": this.options.sym.picHeight,
                            "width": this.options.sym.picWidth
                        });
                        this.graphics.add(new esri.Graphic(onMapPix, sym, {
                            extent: gridExtent,
                            count: pointCount,
                            attr: dataArray[key].attr,
                            STATUS: dataArray[key].attr.type,
                            POINTNO: dataArray[key].attr.bmid,
                            SIZES: dataArray[key].attr.bmkj,
                            EMBED: dataArray[key].attr.bmtm,
                            LASTYHTIME: dataArray[key].attr.bmyhsj,
                            ADDRESS: dataArray[key].attr.bmdz
                        }));                    
                        // }, infoTemplate));
                    }
                    else {
